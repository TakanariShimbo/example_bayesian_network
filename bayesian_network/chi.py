...
def chi_square_test(col1: str, col2: str, cond_cols: List[str], bin_df: pd.DataFrame) -> float:
    ...
    if col1 in cond_cols:
        raise ValueError(f"The variables col1 or col2 can't be in cond_cols. Found {col1} in cond_cols.")
    if col2 in cond_cols:
        raise ValueError(f"The variables col1 or col2 can't be in cond_cols. Found {col2} in cond_cols.")

    # Step 2: Do contingency test.
    if len(cond_cols) == 0:
        # Do a simple contingency test if there are no conditional variables.
        chi, p_value, dof, _ = stats.chi2_contingency(bin_df.groupby([col1, col2]).size().unstack(col2, fill_value=0), lambda_="pearson")
        ...
        for parsed_cond, parsed_bin_df in bin_df.groupby(cond_cols):
            try:
                chi_i, _, dof_i, _ = stats.chi2_contingency(parsed_bin_df.groupby([col1, col2]).size().unstack(col2, fill_value=0), lambda_="pearson")
                chi += chi_i
                dof += dof_i
            except ValueError:
