...
class ConnectionInfosCollector:
    def __init__(self, n_dim: int, connection_df: pd.DataFrame, connection_checker: ConnectionChecker):
        ...
        self._connection_infos: List[ConnectionInfo] = []
    ...
    def _check_and_append_connection_info(self, column1: str, column2: str, condition_columns: List[str] = []):
        ...
        self._connection_infos.append(connection_info)

    def _collect_connection_infos_recursive(self, current_columns: List[str], start: int):
        if len(current_columns) == self._n_dim + 2:
            ...
            for column1, column2, condition_columns in combinations:
                self._check_and_append_connection_info(column1=column1, column2=column2, condition_columns=condition_columns)
            return

        for i in range(start, len(self._column_names)):
            next_columns = [*current_columns, self._column_names[i]]
            self._collect_connection_infos_recursive(current_columns=next_columns, start=i + 1)

    def collect(self):
        self._collect_connection_infos_recursive(current_columns=[], start=0)
        return self._connection_infos
